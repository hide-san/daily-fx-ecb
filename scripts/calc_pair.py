...
def load_eur_rates(path: Path = ECB_RAW_PATH) -> pd.DataFrame:
...
def write_dataset_metadata(pair: str, base: str, quote: str, df: pd.DataFrame) -> None:
    ...
    errors = validate_kaggle_metadata(
        ...
        subtitle=cast(str, metadata["subtitle"]),
        keywords=cast(list[str], metadata["keywords"]),
    )
    for msg in errors:
...
def main() -> None:
    ...
    print(f"Rows computed  : {len(df):,}  ({df['date'].min().date()} to {df['date'].max().date()})")
